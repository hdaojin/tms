...
技能树版本表达某一时期 Skill 的组织结构。每个节点只表示一个稳定 Skill 在该版本、该技术领域中的位置、父子关系和顺序；所有节点都关联 Skill，树不限固定层级，不存在分类、主题或分组节点类型。

树父子关系只表达组织与细化，不自动产生评分、掌握度或 Skill 属性的汇总与继承。同一 Skill 在同一技能树版本中只出现一次，但可以出现在不同版本中。

当前标准技能树是日常维护入口；每个技术领域通过独立页面展示同一版本中的本领域结构。历史版本仍按版本和技术领域查看。
