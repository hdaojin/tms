...
长期稳定的专业项目边界，例如“网络系统管理”。项目代码创建后不可修改，展示名称可以维护。标准、评测和训练数据都必须归属同一技能项目。系统可显式设置至多一个启用项目为默认项目；默认值只简化新建表单选择，不覆盖编辑对象、页面业务上下文或 Service 已确定的项目归属。
...
一次正式竞赛、选拔赛、交流赛、模拟赛、训练考核或训练测试。评测代码创建后不可修改，展示名称可以维护。可绑定系列、级别和训练周期。

### AssessmentModule（评测模块）

某次 Assessment 的实际模块。模块代码创建后不可修改，展示名称可以维护。模块可覆盖一个或多个 TechnicalDomain，但模块本身不是 TechnicalDomain。
...
Assessment 或 AssessmentModule 拥有的私有业务文件，包括试题、评分标准、技术文件、评分脚本、结果文件和附件。新上传资料在同一评测、模块和资料类型下使用严格递增的一位小数版本，首次至少 1.0；相同内容允许以更高版本保存。历史版本不参与比较，原名称保持不变。文件名使用评测、技能项目和模块代码，公共资料模块标识为 GEN，并包含类型、版本及资料日期。评分方案只能由绑定到目标 AssessmentModule 的评分标准生成，评分表不能作为评分方案来源。
