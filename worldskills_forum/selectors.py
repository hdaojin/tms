...
from .models import ForumPost, ForumTopicReadState, Importance, PostType, SourceRole
...
def get_published_post_feed(user, filters):
    ...
    queryset = (
        ...
        .select_related("topic", "topic__module", "topic__category", "translation", "created_by")
    ...
    if q:
        queryset = queryset.filter(
            ...
            | Q(topic__summary__icontains=q) | Q(author_name__icontains=q)
    ...
    if view == "important":
        ...
        queryset = queryset.filter(Q(source_role=SourceRole.WORLDSKILLS_OFFICIAL) | Q(post_type__in=[PostType.OFFICIAL_REPLY, PostType.OFFICIAL_NOTICE, PostType.RULE_CHANGE]))
    elif view == "unread":
...
def get_topic_timeline(topic):
    return topic.posts.filter(translation__isnull=False).select_related("translation", "created_by", "updated_by").prefetch_related("attachments").order_by("posted_at", "pk")
