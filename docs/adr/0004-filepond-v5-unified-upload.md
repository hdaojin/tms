...
4. 为保证拖放和剪贴板产生的 `File` 也能进入普通 Django 表单提交，集成 FilePond v5 的 `FileInputStore`，将 FilePond entries 同步回原生 `<input type="file">`。
5. 启用 `ClipboardSource`，并限制 paste scope，避免多个上传组件同时接收同一次粘贴，也避免干扰 textarea/input/contenteditable 的普通文本粘贴。
6. 图片预览使用 FilePond v5 官方 template 能力（`appendEntryImageView`），不自行维护另一套预览组件。
7. 初始前端扩展集合保持克制：Clipboard、FileInputStore、单文件大小、文件数量，以及图片预览；异步上传、分块上传、图片编辑/压缩等只在实际业务需要时再启用。
