...
from core.constants import BEHAVIORS_UPLOAD_DIR


OLD_APP_LABEL = "conduct"
NEW_APP_LABEL = "behaviors"
...
class Command(BaseCommand):
    help = "把旧 conduct app 的数据库元数据与公共附件目录切换到 behaviors。默认仅预检查，追加 --execute 才会实际修改。"
    ...
    def handle(self, *args, **options):
        ...
        if not has_actions:
            self.stdout.write(self.style.SUCCESS("当前数据库与文件目录已经使用 behaviors，无需切换。"))
            return
        ...
        self.stdout.write(self.style.SUCCESS("conduct 已切换为 behaviors。接下来请运行 manage.py migrate。"))
    ...
    def _get_table_plan(self):
        return [
            TableRename(
                ...
                ConductCategory._meta.db_table,
                ConductCategory,
            ),
            TableRename(
                "ConductSeverityRule",
                "conduct_conductseverityrule",
                ConductSeverityRule._meta.db_table,
                ConductSeverityRule,
            ),
            TableRename(
                "ConductItem",
                "conduct_conductitem",
                ConductItem._meta.db_table,
                ConductItem,
            ),
            TableRename(
                "ConductRecord",
                "conduct_conductrecord",
                ConductRecord._meta.db_table,
                ConductRecord,
            ),
            TableRename(
                "ConductSummary",
                "conduct_conductsummary",
                ConductSummary._meta.db_table,
                ConductSummary,
            ),
        ]
    ...
    def _collect_media_action(self):
        ...
        new_dir = Path(settings.MEDIA_ROOT) / BEHAVIORS_UPLOAD_DIR.name
