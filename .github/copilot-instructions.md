...
- 首次设置：`uv sync`、`npm install`、复制 `.env.example` 到 `.env`、`uv run manage.py migrate`、`uv run manage.py loaddata core/default accounts/default competitions/default behaviors/default`。
