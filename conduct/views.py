...
class ConductRecordCreateView(TitleMixin, PermissionRequiredMixin, CreateView):
    ...
    template_name = 'conduct/conductrecord_create.html'
    success_url = reverse_lazy('conduct:conductrecord_list')
...
class ConductRecordListView(TitleMixin, SingleTableView):
    ...
    template_name = 'conduct/conductrecord_list.html'
    ...
    def get_queryset(self):
        ...
        if not self.request.user.has_perm('conduct.view_all_conduct_records'):
...
class ConductSummaryListView(TitleMixin, SingleTableView):
    ...
    template_name = 'conduct/conductsummary_list.html'
    table_pagination = {"per_page": 20}
    title = "奖惩汇总"
